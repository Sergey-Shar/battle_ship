 ...
 import { addListener, isUnderPoint } from '../additional';
 import ShotView from '../components/ShipView';
 ...
 export default class ComputerScene extends Scene {
	...
	update() {
		...
		if (isEnd) {
			if (opponent.loser) {
				this.status.textContent = "You've lost 👎";
			} else {
		...
		if (this.playerTurn) {
			...
			this.status.textContent =  "Computer move:";
		}
	}
}